<template>
  <transition name="showRouter">
    <div class="find">
    <div class="search-input">

        <div class="input">
          <i class="icon-search"></i>
          <input @keyup.enter="toSearch(keywords)" v-model="keywords" @focus="inputFocus" type="text" placeholder="搜索歌曲" v-bind:class="{'input-focus': !isShowHot}">
          <i @click="keywords=''" v-show="keywords!=='' && !isShowHot" class="icon-cancel"></i>
          <div v-show="!isShowHot" @click="isShowHot=true" class="cancel-btn">取消</div>
        </div>

    </div>
      <div v-if="isShowHot" class="hot">
        热门搜索
        <div class="keywords">
          <div v-for="(item,index) of hotKeywords" @click="toSearch(item)" v-text="item" v-bind:key="index" class="keyword"></div>
        </div>
      </div>
      <div v-else class="search-list">
        <!-- 搜索历史 -->
        <div v-show="isShowHistory" v-for="(item, index) of searchHistory" v-bind:key="index" class="history">
          <div class="icon"><i class="icon-history"></i></div>
          <div class="word" @click="toSearch(item)">{{item}}</div>
          <div class="icon"><i class="icon-del" @click="searchHistory.splice(index,1)"></i></div>
        </div>
        <div v-show="isShowHistory&&searchHistory.length" @click="searchHistory=[]" class="tips">清除搜索记录</div>

        <div class="loading" v-show="isLoading"><i class="icon-loading"></i>搜索中...</div>

        <div @click="playMusic(index, item)" class="music" v-for="(item, index) of musicList" v-bind:key="index">
          <div class="icon-music">
            <img :src="item.album.picUrl">
          </div>
          <div class="music-info">
            <div class="music-name">{{item.name}}</div>
            <div class="music-singer">{{item.artists[0].name}}</div>
            <i v-show="playIndex === index" class="icon-listening"></i>
          </div>
        </div>
        <div v-show="musicList.length" class="tips">没有更多结果了～</div>
      </div>
  </div>
  </transition>
</template>

<script>
import axios from 'axios';

export default {
  name: 'find',
  data() {
    return {
      isShowHot: true,
      playIndex: null,
      keywords: '',
      hotKeywords: [],
      musicList: [],
      isLoading: false,
      isShowHistory: false,
      searchHistory: (localStorage.searchHistory && JSON.parse(localStorage.searchHistory)) || [],
    };
  },
  watch: {
    searchHistory: {
      handler(val) {
        localStorage.searchHistory = JSON.stringify(val);
      },
      deep: true,
    },
  },
  computed: {
    DOM() {
      return this.$store.state.DOM;
    },
    isPlaying() {
      return this.$store.state.isPlaying;
    },
  },
  methods: {
    inputFocus() {
      if (!this.keywords.trim()) {
        this.isShowHot = false;
        this.isShowHistory = true;
        this.musicList = [];
      }
    },
    unique(arr, keyword) {
      let isRepeat = false;
      for (let i = 0; i < arr.length; i += 1) {
        if (arr[i] === keyword) {
          isRepeat = true;
          break;
        }
      }
      if (!isRepeat) {
        arr.unshift(keyword);
      }
      return arr;
    },
    async toSearch(keyword) {
      if (keyword.trim()) {
        this.isShowHot = false;
        this.isShowHistory = false;
        this.isLoading = true;
        const param = {
          params: {
            keywords: keyword,
            type: 1,
          },
        };
        const resp = await axios.get('/api/search', param);
        console.log(resp.data);
        window.resp = resp;
        if (resp.data.code === 200 && resp.data.result !== '') {
          this.isLoading = false;
          this.musicList = resp.data.result.songs;
          this.searchHistory = this.unique(this.searchHistory, keyword);
        }
      }
    },
    async playMusic(index, item) {
      if (this.playIndex === index) {
        this.DOM.audio.paused ? this.DOM.audio.play() : this.DOM.audio.pause();
        this.$store.commit('play', !this.isPlaying);
      } else {
        this.playIndex = index;
        const param = {
          params: {
            id: item.id,
          },
        };
        const resp = await axios.get('/api/music/url', param);
        if (resp.data.code === 200 && resp.data.data[0]) {
          console.log(resp);
          const payload = {
            id: item.id,
            name: item.name,
            musicImgSrc: item.album.picUrl,
            src: resp.data.data[0].url,
          };
          this.$store.commit('playMusic', payload);
          this.$store.commit('addMusic', payload);
        }
      }
    },
  },
  beforeCreate() {
    this.$store.commit('showMiniMusic', true);
  },
  async created() {
    const param = {
      params: {
        limit: 6,
      },
    };
    const resp = await axios.get('/api/top/artists', param);
    console.log(resp.data.artists);
    this.hotKeywords = resp.data.artists.map(item => item.name);
    console.log(this.hotKeywords);
  },
  mounted() {
    this.$store.commit('changeLinkBorderIndex', 2);
  },
};
</script>

<style lang="scss" scoped>
.showRouter-enter-active {
  transition: all .4s ease;
}
.showRouter-leave-active {
  transition: all 0 ease;
}
.showRouter-enter, .showRouter-leave-active {
  transform: translateX(150px);
  opacity: 0;
}
  .find {
    display: flex;
    flex-direction: column;
    width: 100%;
    height: 100%;
    flex: 10;
    overflow: auto;

    .search-input {
        position: relative;
        background-color: rgba(0, 0, 0, .1);
        text-align: center;
        border-bottom: 1px solid rgba(0, 0, 0, .1);

        .input {
            position: relative;
            width: 90%;
            height: 35px;
            margin: 20px auto;
            text-align: left;

            i.icon-search {
                position: absolute;
                top: 7px;
                left: 6px;
                display: inline-block;
                width: 20px;
                height: 20px;
                background: url('./find.svg') no-repeat;
                background-size: contain;
            }
            input {
                border-radius: 3px;
                border: none;
                padding: 0 34px;
                width: 100%;
                height: 100%;
                font-size: 1rem;
                outline: none;
            }
            i.icon-cancel {
                position: absolute;
                top: 7px;
                right: 21%;
                display: inline-block;
                height: 20px;
                width: 20px;
                background: url('./cancel.svg') no-repeat;
                background-size: contain;
            }
            .input-focus {
                width: 80%;
            }
            .cancel-btn {
                vertical-align: top;
                padding: 7px 10px;
                height: 100%;
                background-color: none;
                display: inline-block;
            }
        }
    }

    .hot {
      flex: 8;
      padding: 10px;
      .keywords {
        display: flex;
        flex-wrap: wrap;
        padding-top: 10px;
        padding-left: 16px;
        .keyword:hover {
          border: 1px solid orange;
        }
        div {
          text-align: center;
          width: 25%;
          padding: 5px;
          border: 1px solid gray;
          margin-right: 8%;
          margin-bottom: 10px;
          border-radius: 20px;
          cursor: pointer;
        }
        div:nth-of-type(1) {
          border-color: red;
          box-shadow: none;
          color: red;
        }
      }
    }

    .search-list {
      flex: 9;
      overflow: auto;
      .history + .history {
        border-top: 1px solid rgba(0, 0, 0, .1);
      }
      .history {
        display: flex;
        width: 100%;
        height: 50px;
        justify-content: center;
        align-items: center;

        .icon {
          flex: 1;
          height: 100%;
          display: flex;
          justify-content: center;
          align-items: center;
        }
        .icon-history {
          display: inline-block;
          width: 23px;
          height: 23px;
          background: url('./history.svg') no-repeat;
          background-size: contain;
        }
        .icon-del {
          display: inline-block;
          width: 20px;
          height: 20px;
          background: url('./del.svg') no-repeat;
          background-size: contain;
        }
        .word {
          flex: 5;
          width: 80%;
          overflow: hidden;
          text-overflow: ellipsis;
          white-space: nowrap;
          cursor: pointer;
        }
      }
      .tips {
        text-align: center;
        margin: 12px auto;
        width: 200px;
        font-size: 80%;
        color: gray;
      }
      .loading {
        padding-top: 10px;
        text-align: center;

        .icon-loading {
          display: inline-block;
          margin: auto;
          width: 22px;
          height: 22px;
          background: url('./loading.svg') no-repeat;
          background-size: contain;
          animation: loading .6s linear infinite;
          vertical-align: top;
          margin-right: 10px;
        }
      }
      .music + .music {
        border-top: 1px solid rgba(0, 0, 0, .1);
      }
      .music {
        display: flex;
        width: 100%;
        height: 60px;
        cursor: pointer;
        .icon-music {
          display: flex;
          justify-content: center;
          align-items: center;
          width: 60px;
          height: 60px;

          img {
            display: inline-block;
            width: 90%;
            height: 90%;
            background: url('./music.svg') no-repeat;
            background-size: contain;
          }
        }
        .music-info {
          position: relative;
          flex: 1;
          padding: 8px 10px;
          .music-name {
            width: 250px;
            text-overflow: ellipsis;
            white-space: nowrap;
            overflow: hidden;
          }
          .music-singer {
            padding-top: 3px;
            font-size: 50%;
            color: gray;
          }
          i {
            position: absolute;
            display: inline-block;
            width: 13px;
            height: 13px;
          }
          i.icon-listening {
            background: url('./listening.svg') no-repeat;
            background-size: contain;
            right: 20px;
            top: 22px;
            animation: listening 1s linear infinite;
          }
        }
      }
    }
  }
  @keyframes loading {
    from {
      transform: rotate(0);
    }
    to {
      transform: rotate(360deg)
    }
  }
  @keyframes listening {
    0% {
      transform: scale(1);
    }
    33% {
      transform: scale(1.3);
    }
    66% {
      transform: scale(1);
    }
  }
</style>
